...
public class ImageRepository {
  ...
  // updated method to get image by id instead of title
  public Image getImageById(Integer id) {
    EntityManager em = emf.createEntityManager();
    try {
      TypedQuery<Image> typedQuery =
          em.createQuery("SELECT i from Image i where i.id =:id", Image.class)
              .setParameter("id", id);
  ...
  */
  ...
  // This changes the state of the image model from detached state to persistent state, which is
  // very essential to use the remove() method
  // If you use remove() method on the object which is not in persistent state, an exception is
  // thrown
  // The transaction is committed if it is successful
  // The transaction is rolled back in case of unsuccessful transaction
