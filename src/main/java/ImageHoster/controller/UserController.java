...
public class UserController {
  ...
  // This controller method is called when the request pattern is of type 'users/login' and also the
  // incoming request is of POST type
  // The return type of the business logic is changed to User type instead of boolean type. The
  // login() method in the business logic checks whether the user with entered username and password
  // exists in the database and returns the User type object if user with entered username and
  // password exists in the database, else returns null
  // If user with entered username and password exists in the database, add the logged in user in
  // the Http Session and direct to user homepage displaying all the images in the application
  ...
  // 'index.html' file is returned showing the landing page of the application and displaying all
